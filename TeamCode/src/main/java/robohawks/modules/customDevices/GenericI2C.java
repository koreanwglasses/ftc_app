...
/**
 * Created by fchoi on 2/3/2017.
 */
public class GenericI2C implements I2cController.I2cPortReadyCallback{
    // --------------------------------- CONSTANTS -------------------------------------------------
...
// --------------------------------- CLASS INIT AND CLOSE ----------------------------------------
    ...
    public GenericI2C(HardwareMap hardwareMap, String deviceName, I2cAddr addr) {
        transferQueue       = new ArrayQueue<I2cTransfer>();
        csDev               = hardwareMap.i2cDevice.get(deviceName);
        csDevAddr           = (byte) addr.get8Bit();
...
//                    wCache[0], wCache[1], wCache[2], wCache[3]));
    ...
    public void addWriteRequest(byte regNumber,byte regValue) {
...
//Log.i(logId, String.format("aWR reg= 0x%02X, val= 0x%02X, qLength= %d", regNumber, regValue, transferQueue.length()));
    ...
    public void addWriteRequest(byte regNumber, byte regCount, long regValue) {
        addRequest(new I2cTransfer(regNumber, regCount, regValue, true));
    }

    public void addWriteRequest(byte regNumber, byte regCount, long regValue, boolean isLowFirst) {
        addRequest(new I2cTransfer(regNumber, regCount, regValue, isLowFirst));
    }

    public void addReadRequest(byte regNumber,byte regCount) {
...
//Log.i(logId, String.format("aRR reg= 0x%02X, cnt= 0x%02X, qLength= %d", regNumber, regCount, transferQueue.length()));
    ...
    public void addRequest(I2cTransfer element) {
        try {
...
// --------------------------------- PROCESSING OF RECEIVED DATA ---------------------------------
    ...
    public int getWord(int lowByte, int highByte) {
